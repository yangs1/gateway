...
// Viper //
// 优先级: 命令行 > 环境变量 > 默认值
func Viper(rawVal interface{}, path ...string) *viper.Viper {
	var cfg string
	...
	if len(path) == 0 {
		...
		if cfg == "" {
			...
			*/
			if configEnv := os.Getenv(autoload.ConfigEnv); configEnv == "" {
				switch gin.Mode() {
				case gin.DebugMode:
					cfg = autoload.ConfigDebugFile
				case gin.ReleaseMode:
					cfg = autoload.ConfigReleaseFile
				case gin.TestMode:
					cfg = autoload.ConfigTestFile
				default:
					cfg = autoload.ConfigDefaultFile
				}
				fmt.Printf("正在使用gin模式的%s环境名称,config的路径为%s\n", gin.EnvGinMode, cfg)
			} else {
				// internal.ConfigEnv 常量存储的环境变量不为空 将值赋值于config
				cfg = configEnv
				fmt.Printf("正在使用%s环境变量,config的路径为%s\n", autoload.ConfigEnv, cfg)
			}
		} else {
			cfg = "config/app.yaml"
